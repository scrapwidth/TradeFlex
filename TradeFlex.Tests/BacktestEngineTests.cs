...
public class BacktestEngineTests
{
    ...
    public async Task EngineRunsThroughBars()
    {
        ...
        var result = await engine.RunAsync(algo, "engine_fixture.parquet", "SAMPLE");

        Assert.Equal(2, algo.Count);
        Assert.Empty(result.Trades);
        Assert.Equal(100000m, result.InitialCash);
        Assert.Equal(100000m, result.FinalCash);
        Assert.Equal(0m, result.TotalReturnPercent);
        Assert.Equal(0m, result.MaxDrawdownPercent);
    }

    [Fact]
    public async Task EngineFiltersBarsByFromDate()
    {
        var dataDir = Path.Combine(AppContext.BaseDirectory, "data");
        Directory.CreateDirectory(dataDir);
        var file = Path.Combine(dataDir, "engine_from_filter.parquet");

        var bars = new[]
        {
            new Bar("SAMPLE", new DateTime(2024,1,1,0,0,0,DateTimeKind.Utc),1,1,1,1,1),
            new Bar("SAMPLE", new DateTime(2024,1,1,0,1,0,DateTimeKind.Utc),2,2,2,2,1),
            new Bar("SAMPLE", new DateTime(2024,1,1,0,2,0,DateTimeKind.Utc),3,3,3,3,1),
        };
        ...
        // Filter out the first bar using from parameter
        var fromDate = new DateTime(2024, 1, 1, 0, 1, 0, DateTimeKind.Utc);
        var result = await engine.RunAsync(algo, "engine_from_filter.parquet", "SAMPLE", from: fromDate);

        // Only 2 bars should be processed (the first one is filtered out)
        Assert.Equal(2, algo.Count);
    }

    [Fact]
    public async Task EngineFiltersBarsByToDate()
    {
        var dataDir = Path.Combine(AppContext.BaseDirectory, "data");
        Directory.CreateDirectory(dataDir);
        var file = Path.Combine(dataDir, "engine_to_filter.parquet");
        ...
        // Filter out the last bar using to parameter
        var toDate = new DateTime(2024, 1, 1, 0, 1, 0, DateTimeKind.Utc);
        var result = await engine.RunAsync(algo, "engine_to_filter.parquet", "SAMPLE", to: toDate);

        // Only 2 bars should be processed (the last one is filtered out)
        Assert.Equal(2, algo.Count);
    }
}
