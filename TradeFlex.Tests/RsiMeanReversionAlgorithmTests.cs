...
public class RsiMeanReversionAlgorithmTests
{
    ...
    [Fact]
    public async Task DefaultConstructor_UsesDefaultParameters()
    {
    ...
    [Fact]
    public async Task CustomParameters_AreRespected()
    {
    ...
    [Fact]
    public async Task OnBar_FirstBar_NoTradeNoPreviousClose()
    {
        var broker = new PaperBroker(10000m, feePercentage: 0m);
        var context = new TestContext(broker);
    ...
    public async Task OnBar_NotEnoughData_NoTrades()
    {
        ...
        await algo.OnBarAsync(CreateBar(100m));
        await algo.OnBarAsync(CreateBar(101m));
        await algo.OnBarAsync(CreateBar(102m));

        Assert.Empty(broker.Trades);
    }

    [Fact]
    public async Task OversoldCondition_GeneratesBuySignal()
    {
        var broker = new PaperBroker(10000m, feePercentage: 0m);
        var context = new TestContext(broker);
        var algo = new RsiMeanReversionAlgorithm(3, 30m, 70m);
        ...
        broker.UpdatePrice(Symbol, 100m);
        await algo.OnBarAsync(CreateBar(100m)); // First bar, establishes previous close

        broker.UpdatePrice(Symbol, 90m);
        await algo.OnBarAsync(CreateBar(90m)); // Change: -10

        broker.UpdatePrice(Symbol, 80m);
        await algo.OnBarAsync(CreateBar(80m)); // Change: -10

        broker.UpdatePrice(Symbol, 70m);
        await algo.OnBarAsync(CreateBar(70m)); // Change: -10
    ...
    [Fact]
    public async Task OverboughtCondition_GeneratesSellSignal()
    {
        var broker = new PaperBroker(10000m, feePercentage: 0m);
        var context = new TestContext(broker);
        var algo = new RsiMeanReversionAlgorithm(3, 30m, 70m);
        ...
        broker.UpdatePrice(Symbol, 90m);
        await algo.OnBarAsync(CreateBar(90m));

        broker.UpdatePrice(Symbol, 80m);
        await algo.OnBarAsync(CreateBar(80m));

        broker.UpdatePrice(Symbol, 70m);
        await algo.OnBarAsync(CreateBar(70m));

        // Should have bought
        Assert.True(await broker.GetPositionAsync(Symbol) > 0);
        ...
        broker.UpdatePrice(Symbol, 100m);
        await algo.OnBarAsync(CreateBar(100m)); // Change: +10
    ...
    [Fact]
    public async Task Rsi100_AllGains_SellSignalIfPosition()
    {
        var broker = new PaperBroker(10000m, feePercentage: 0m);
        var context = new TestContext(broker);
        var algo = new RsiMeanReversionAlgorithm(3, 30m, 70m);
        ...
        await broker.SubmitOrderAsync(new Order(Symbol, 100m, 0)); // Buy 100 shares

        // Now run algo with rising prices (RSI = 100)
        await algo.OnBarAsync(CreateBar(50m));

        broker.UpdatePrice(Symbol, 60m);
        await algo.OnBarAsync(CreateBar(60m)); // +10

        broker.UpdatePrice(Symbol, 70m);
        await algo.OnBarAsync(CreateBar(70m)); // +10

        broker.UpdatePrice(Symbol, 80m);
        await algo.OnBarAsync(CreateBar(80m)); // +10
    ...
    [Fact]
    public async Task Rsi0_AllLosses_BuySignal()
    {
        var broker = new PaperBroker(10000m, feePercentage: 0m);
        var context = new TestContext(broker);
        var algo = new RsiMeanReversionAlgorithm(3, 30m, 70m);
    ...
    [Fact]
    public async Task RsiMidRange_NoTrades()
    {
        var broker = new PaperBroker(10000m, feePercentage: 0m);
        var context = new TestContext(broker);
        ...
        await algo.OnBarAsync(CreateBar(105m)); // +5

        broker.UpdatePrice(Symbol, 100m);
        await algo.OnBarAsync(CreateBar(100m)); // -5
    ...
    [Fact]
    public async Task WindowMaintainsCorrectSize()
    {
        var broker = new PaperBroker(10000m, feePercentage: 0m);
        var context = new TestContext(broker);
        var algo = new RsiMeanReversionAlgorithm(3, 30m, 70m);
        ...
        await algo.OnBarAsync(CreateBar(70m));
        await algo.OnBarAsync(CreateBar(60m));
        await algo.OnBarAsync(CreateBar(50m)); // 6 bars, but window should only hold 3
